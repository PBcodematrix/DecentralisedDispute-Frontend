...
contract ArbitrationSystem {
    ...
    mapping(address => uint256) public jurorSelectionCounts;
    ...
    function selectJurors() external returns (SelectJuror[] memory) {
        ...
        if (jurorCount <= 10) {
            // Case 1: Less than or equal to 10 jurors, assign weights directly
            ...
            // Case 2: More than 10 jurors, perform random selection
            // SelectJuror;
            uint256 totalWeight = 0;

            // Step 1: Accumulate total weight based on stakes
            for (uint256 i = 0; i < jurors.length; i++) {
                totalWeight += jurors[i].stake;
            }

            // Step 2: Map to track selection counts

            // Step 3: Randomly select jurors based on their stake
            for (uint256 i = 0; i < 10; i++) {
                uint256 randomIndex = uint256(keccak256(abi.encodePacked(block.timestamp, block.prevrandao, i))) % totalWeight;
                uint256 accumulatedWeight = 0;

                for (uint256 j = 0; j < jurors.length; j++) {
                    accumulatedWeight += jurors[j].stake;

                    if (randomIndex < accumulatedWeight) {
                        jurorSelectionCounts[jurors[j].addr]++;
                        break;
                    }
                }
            }

            // Step 4: Populate selectedJurors array
            uint256 selectedIndex = 0;
            for (uint256 i = 0; i < jurors.length && selectedIndex < 10; i++) {
                if (jurorSelectionCounts[jurors[i].addr] > 0) {
                    selectedJurors[selectedIndex] = SelectJuror({
                        pubKey: jurors[i].addr,
                        weight: jurorSelectionCounts[jurors[i].addr]
                    });
                    selectedIndex++;
                }
            }
